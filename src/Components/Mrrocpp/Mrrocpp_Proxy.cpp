...
Mrrocpp_Proxy::Mrrocpp_Proxy(const std::string & name) :
  Base::Component(name), state(MPS_NOT_INITIALIZED), port("port", 8888, "range")
{
  LOG(LNOTICE) << "Mrrocpp_Proxy::Mrrocpp_Proxy\n";
  LOG(LNOTICE) << port << "\n";
...
{
  LOG(LNOTICE) << "Mrrocpp_Proxy::onStart\n";
  return true;
}
...
{
  LOG(LNOTICE) << "Mrrocpp_Proxy::onInit\n";
...
{
  LOG(LNOTICE) << "Mrrocpp_Proxy::onStop\n";
  return true;
}

bool Mrrocpp_Proxy::onFinish()
{
  LOG(LNOTICE) << "Mrrocpp_Proxy::onFinish\n";
...
void Mrrocpp_Proxy::tryAcceptConnection()
{
  ...
  if (!serverSocket.isDataAvailable(acceptConnectionTimeout)) {
    LOG(LNOTICE) << "if (!serverSocket.isDataAvailable()) {\n";
    return;
  }
...
{
  LOG(LNOTICE) << "Mrrocpp_Proxy::receiveBuffersFromMrrocpp() begin\n";
